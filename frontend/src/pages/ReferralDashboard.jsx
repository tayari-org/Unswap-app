...
export default function ReferralDashboard() {
    ...
    if (isLoading) return <div className="p-8 text-center text-slate-500 font-black uppercase tracking-widest">Accessing Security Dossier...</div>;
        ...
        <div className="min-h-screen bg-slate-50 pb-20">
            ...
            <div className="bg-[#1E293B] pt-16 pb-24 relative overflow-hidden">
                ...
                <div className="max-w-4xl mx-auto px-6 relative z-10 text-center">
                    <Badge className="bg-indigo-600/20 text-indigo-400 border-indigo-500/30 mb-6 uppercase tracking-[0.2em] px-4 py-1">
                        Exclusive Network Growth
                    </Badge>
                    <h1 className="text-4xl md:text-6xl font-bold text-white tracking-tighter mb-6">
                        The Founders' <span className="text-indigo-400 italic">Lifetime Waiver</span>
                    </h1>
            ...
            <div className="max-w-5xl mx-auto px-6 -mt-12 space-y-8">
                    ...
                    <CardContent className="p-8 md:p-12">
                        <div className="grid md:grid-cols-2 gap-12 items-center">
                            <div className="space-y-6">
                                <div>
                                    <h3 className="text-[11px] font-black uppercase tracking-[0.3em] text-indigo-600 mb-2">Your Diplomatic Credential</h3>
                                ...
                                <div className="flex gap-2">
                                    <div className="relative flex-1">
                                        <Input
                                            ...
                                            className="bg-slate-50 border-slate-200 h-14 pl-4 pr-12 font-mono text-sm text-slate-600 rounded-xl"
                                        />
                                        <button
                                    ...
                                    <Button size="icon" className="h-14 w-14 rounded-xl bg-slate-900 hover:bg-slate-800 shrink-0">
                            ...
                            <div className="bg-slate-50 rounded-3xl p-8 border border-slate-100 flex flex-col items-center justify-center text-center">
                                ...
                                <span className="text-5xl font-mono font-medium tracking-tighter text-slate-900 italic">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {milestones.map((m) => (
                        <Card key={m.id} className={`border-slate-200 rounded-3xl overflow-hidden transition-all duration-500 ${m.isUnlocked ? 'bg-indigo-600 text-white border-indigo-400 shadow-xl shadow-indigo-200' : 'bg-white hover:border-slate-300 shadow-sm'}`}>
                            <CardContent className="p-6 space-y-4">
                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${m.isUnlocked ? 'bg-white/20' : 'bg-slate-50 text-slate-400'}`}>
                                    <m.icon className="w-6 h-6" />
                                </div>
                                <div>
                                    <Badge variant="outline" className={`mb-2 font-black text-[8px] uppercase tracking-widest ${m.isUnlocked ? 'border-white/30 text-white' : 'text-slate-400'}`}>
                                        {m.requirement} {m.requirement === 1 ? 'Referral' : 'Referrals'}
                                    </Badge>
                                    <h4 className="font-bold text-lg leading-tight">{m.name}</h4>
                                    <p className={`text-xs mt-2 font-medium leading-relaxed ${m.isUnlocked ? 'text-indigo-100' : 'text-slate-400'}`}>
                                        {m.reward}
                                    </p>
                                </div>
                                {m.isUnlocked && (
                ...
                <Card className="border-slate-200 shadow-sm rounded-3xl overflow-hidden bg-white">
                    <CardHeader className="border-b border-slate-50 px-8 py-6">
                        <CardTitle className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-800">Operational Network</CardTitle>
                    </CardHeader>
                    <CardContent className="p-0">
                        {colleagues.length === 0 ? (
                            <div className="text-center py-16 text-slate-300">
                                <p className="font-bold text-[10px] uppercase tracking-widest">No recruits recorded in current sector.</p>
                            </div>
                        ) : (
                                ...
                                {colleagues.map((c) => (
                                    <div key={c.id} className="flex items-center justify-between px-8 py-4 hover:bg-slate-50 transition-colors">
                                        <div className="flex items-center gap-4">
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${c.status === 'verified' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
                                                <Users className="w-5 h-5" />
                                            </div>
                                            <span className="font-mono text-sm text-slate-600 uppercase italic font-medium">{c.email_masked}</span>
                                        </div>
                                        <Badge variant="outline" className={`capitalize font-black text-[9px] tracking-widest ${c.status === 'verified' ? 'text-emerald-600 border-emerald-100 bg-emerald-50' :
                                                c.status === 'registered' ? 'text-indigo-600 border-indigo-100 bg-indigo-50' :
                                                    'text-slate-400'
                                            }`}>
                                            {c.status}
                                        </Badge>
